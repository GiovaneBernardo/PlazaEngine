...
namespace Plaza {
	std::string AssetsImporter::ImportAsset(std::string path, uint64_t uuid, AssetsImporterSettings settings) {
		...
		std::string outDirectory =
			settings.outDirectory.empty() ? Editor::Gui::FileExplorer::currentDirectory : settings.outDirectory;
		std::string outPath = outDirectory + "/" +
							  Editor::Utils::Filesystem::GetUnrepeatedName(std::filesystem::path{path}.stem().string());
	...
	void AssetsImporter::ImportModel(const AssetImported& asset, const std::string& outPath,
									 const std::string& outDirectory, const AssetsImporterSettings& settings) {
		...
		switch (AssetsImporter::mExtensionMapping.at(extension)) {
			...
			case AssetExtension::FBX:
				if (settings.mImportModel) {
					modelScene =
						AssetsImporter::ImportFBX(asset, std::filesystem::path(outPath), *model.get(), settings);
				}
				// AssetsLoader::LoadPrefab(AssetsManager::NewAsset(AssetType::MODEL, outPath +
		...
		AssetsSerializer::SerializeFile<Model>(*model.get(), model->mAssetPath.string(),
											   Application::Get()->mSettings.mModelSerializationMode);
		AssetsSerializer::SerializeFile<Prefab>(*prefab.get(), prefab->mAssetPath.string(),
												Application::Get()->mSettings.mPrefabSerializationMode);

		std::shared_ptr<Model> modelAsset = AssetsSerializer::DeSerializeFile<Model>(
			model->mAssetPath.string(), Application::Get()->mSettings.mModelSerializationMode);
		AssetsLoader::LoadAsset(modelAsset.get());
		std::shared_ptr<Prefab> prefabAsset = AssetsSerializer::DeSerializeFile<Prefab>(
			prefab->mAssetPath.string(), Application::Get()->mSettings.mPrefabSerializationMode);
	...
	void AssetsImporter::ImportAnimation(std::filesystem::path filePath, std::filesystem::path outFolder,
		...
		for (Animation& animation : loadedAnimations) {
			std::string animationOutPath = Editor::Utils::Filesystem::GetUnrepeatedPath(
				outFolder.string() + "/" + animation.mAssetName + Standards::animationExtName);
	...
	std::string AssetsImporter::ImportTexture(AssetImported importedAsset, uint64_t uuid) {
		std::string outPath = Editor::Gui::FileExplorer::currentDirectory + "/" +
							  std::filesystem::path{importedAsset.mPath}.filename().string();
		outPath = Editor::Utils::Filesystem::GetUnrepeatedPath(outPath);

		std::filesystem::copy(importedAsset.mPath, outPath);

		Asset asset{uuid <= 0 ? Plaza::UUID::NewUUID() : uuid,
					std::filesystem::path{importedAsset.mPath}.filename().string(), outPath};
