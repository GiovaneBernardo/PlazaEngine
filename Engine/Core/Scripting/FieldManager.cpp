...
namespace Plaza {
	std::any FieldManager::GetFieldValue() { return std::any(); }
	...
	std::map<uint64_t, std::map<std::string, std::map<std::string, Field*>>> FieldManager::GetAllScritpsFields(
		...
		std::map<uint64_t, std::map<std::string, std::map<std::string, Field*>>> allFields =
			std::map<uint64_t,
					 std::map<std::string, std::map<std::string, Field*>>>(); // Entity UUID, Class Name, Field Name

		return allFields;
	}

	void FieldManager::ApplyAllScritpsFields(
