...
namespace Plaza {
	...
	class PLAZA_API VulkanMesh : public Mesh {
	  public:
		...
		VulkanMesh(vector<glm::vec3> vertices, vector<glm::vec3> normals, vector<glm::vec2> uvs,
				   vector<glm::vec3> tangent, vector<unsigned int> indices) {
		...
		// VulkanMesh(vector<glm::vec3> vertices, vector<glm::vec3> normals, vector<glm::vec2> uvs, vector<glm::vec3>
		// tangent, vector<glm::vec3> bitangent, vector<unsigned int> indices) { 	this->vertices = vertices;
		//	this->indices = indices;
		//	this->normals = normals;
		//	this->uvs = uvs;
		//	this->tangent = tangent;
		...
		VulkanMesh(vector<glm::vec3> vertices, vector<glm::vec3> normals, vector<glm::vec2> uvs,
				   vector<glm::vec3> tangent, vector<unsigned int> indices, bool usingNormal,
		...
		VulkanMesh() { uuid = Plaza::UUID::NewUUID(); }
	  ...
	  private:
		...
		//	GetVulkanRenderer().CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
