...
namespace Plaza {
	uint8_t VulkanBloom::CalculateMipmapLevels(int m_width, int m_height, int m_max_iterations, int m_downscale_limit) {
		...
		for (uint8_t i = 0; i < m_max_iterations; ++i) {
			...
			if (width < m_downscale_limit || height < m_downscale_limit)
				break;
	...
	void VulkanBloom::InitializeDescriptorSets() {
		...
		for (unsigned int i = 0; i < this->mMipCount; ++i) {
			...
			if (vkCreateDescriptorPool(VulkanRenderer::GetRenderer()->mDevice, &poolInfo, nullptr, &descriptorPool) !=
				VK_SUCCESS) {
				throw std::runtime_error("failed to create descriptor pool!");
			}

			std::vector<VkDescriptorSetLayout> layouts(Application::Get()->mRenderer->mMaxFramesInFlight,
													   mComputeShadersScaleDown.mComputeDescriptorSetLayout);
			...
			if (vkAllocateDescriptorSets(VulkanRenderer::GetRenderer()->mDevice, &allocInfo,
										 this->mDownScaleDescriptorSets[i].data()) != VK_SUCCESS) {
				throw std::runtime_error("failed to allocate descriptor sets!");
			}

			VkDescriptorPool descriptorPool2;
			if (vkCreateDescriptorPool(VulkanRenderer::GetRenderer()->mDevice, &poolInfo, nullptr, &descriptorPool2) !=
				VK_SUCCESS) {
				throw std::runtime_error("failed to create descriptor pool!");
			}
			allocInfo.descriptorPool = descriptorPool2;
			if (vkAllocateDescriptorSets(VulkanRenderer::GetRenderer()->mDevice, &allocInfo,
										 this->mUpScaleDescriptorSets[i].data()) != VK_SUCCESS) {
				throw std::runtime_error("failed to allocate descriptor sets!");
			}
			...
			VkImageView outputImageView =
				VK_NULL_HANDLE; //= pingPong ? this->mTexture1->mImageView : this->mTexture1->mImageView;
			...
			if (i < this->mMipCount - 1) {
				for (unsigned int j = 0; j < Application::Get()->mRenderer->mMaxFramesInFlight; ++j) {
					UpdateDescriptorSet(inputLayout, inputView, inputSampler, outputImageView, j,
										this->mDownScaleDescriptorSets[i][j]);
				}
			}
			...
			for (unsigned int j = 0; j < Application::Get()->mRenderer->mMaxFramesInFlight; ++j) {
				UpdateDescriptorSet(inputLayout, inputView, inputSampler, outputImageView, j,
									this->mUpScaleDescriptorSets[i][j]);
			}
	...
	void VulkanBloom::UpdateDescriptorSet(VkImageLayout inputLayout, VkImageView inputView, VkSampler inputSampler,
										  VkImageView outputImageView, unsigned int frame,
										  VkDescriptorSet& descriptorSet) {
		// VulkanRenderer::GetRenderer()->TransitionImageLayout(VulkanRenderer::GetRenderer()->mFinalSceneImage,
		// VulkanRenderer::GetRenderer()->mSwapChainImageFormat, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR,
		...
		sceneImageInfo.imageView =
			VulkanRenderer::GetRenderer()
				->mLighting->mDeferredEndTexture.mImageView; // VulkanRenderer::GetRenderer()->mDeferredFinalImageView;
		...
		vkUpdateDescriptorSets(VulkanRenderer::GetRenderer()->mDevice,
							   mComputeShadersScaleDown.mDescriptorWrites.size(),
							   mComputeShadersScaleDown.mDescriptorWrites.data(), 0, nullptr);
	}

	void VulkanBloom::UpdateDescriptorSets() {}

	void VulkanBloom::Init() {
		this->mMipCount = this->CalculateMipmapLevels(Application::Get()->appSizes->sceneSize.x,
													  Application::Get()->appSizes->sceneSize.y, 16, 10);
		this->mTexture1 = new VulkanTexture();
		this->mTexture1->mMipCount = this->mMipCount;
		this->mTexture1->CreateTextureImage(VulkanRenderer::GetRenderer()->mDevice, VK_FORMAT_R32G32B32A32_SFLOAT,
											Application::Get()->appSizes->sceneSize.x,
											Application::Get()->appSizes->sceneSize.y, true,
											VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
												VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_STORAGE_BIT);
		VulkanRenderer::GetRenderer()->TransitionImageLayout(mTexture1->mImage, VK_FORMAT_R32G32B32A32_SFLOAT,
															 VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
															 VK_IMAGE_ASPECT_COLOR_BIT, 1, mTexture1->mMipCount);
		this->mTexture1->CreateTextureSampler(VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE, VK_SAMPLER_MIPMAP_MODE_NEAREST,
		...
		VulkanRenderer::GetRenderer()->AddTrackerToImage(this->mTexture1->mImage, "Bloom Downscale",
														 this->mTexture1->mSampler, this->mTexture1->GetTextureInfo(),
		...
		if (vkCreateDescriptorSetLayout(VulkanRenderer::GetRenderer()->mDevice, &layoutInfo, nullptr,
										&this->mComputeShadersScaleDown.mComputeDescriptorSetLayout) != VK_SUCCESS) {
			throw std::runtime_error("failed to create compute descriptor set layout!");
		}

		if (vkCreateDescriptorSetLayout(VulkanRenderer::GetRenderer()->mDevice, &layoutInfo, nullptr,
										&this->mComputeShadersScaleUp.mComputeDescriptorSetLayout) != VK_SUCCESS) {
			throw std::runtime_error("failed to create compute descriptor set layout!");
		}
		...
		pushConstantRange.stageFlags =
			VK_SHADER_STAGE_COMPUTE_BIT;			   // Specify the shader stage(s) using the push constant
		...
		this->mComputeShadersScaleDown.Init(
			Application::Get()->enginePath + "/Shaders/Vulkan/bloom/bloomDownScale.comp", {pushConstantRange});
		this->mComputeShadersScaleUp.Init(Application::Get()->enginePath + "/Shaders/Vulkan/bloom/bloomUpScale.comp",
										  {pushConstantRange});
	}
	...
	void VulkanBloom::Draw() {
		...
		/* Bloom: downscale */
		glm::uvec2 mipSize =
			glm::uvec2(Application::Get()->appSizes->sceneSize.x / 2, Application::Get()->appSizes->sceneSize.y / 2);
		...
		for (uint8_t i = 0; i < mMipCount - 1; ++i) {
			...
			this->mComputeShadersScaleDown.Dispatch(
				glm::ceil(float(mipSize.x) / 8), glm::ceil(float(mipSize.y) / 8), 1, &pushConstant,
				sizeof(PushConstant), this->mDownScaleDescriptorSets[i][Application::Get()->mRenderer->mCurrentFrame]);
		...
		for (uint8_t i = mMipCount - 1; i >= 1; --i) {
			...
			this->mComputeShadersScaleUp.Dispatch(
				glm::ceil(float(mipSize.x) / 8), glm::ceil(float(mipSize.y) / 8), 1, &pushConstant,
				sizeof(PushConstant), this->mUpScaleDescriptorSets[i][Application::Get()->mRenderer->mCurrentFrame]);
	...
	void VulkanBloom::CopySceneTexture() {
		...
		}
		{
			...
			VulkanRenderer::GetRenderer()->TransitionImageLayout(this->mTexture1->mImage, VK_FORMAT_R32G32B32A32_SFLOAT,
																 VK_IMAGE_LAYOUT_GENERAL,
		...
		VkCommandBuffer commandBuffer = VulkanRenderer::GetRenderer()->BeginSingleTimeCommands();
		{
			...
			vkCmdCopyImage(commandBuffer, VulkanRenderer::GetRenderer()->mLighting->mDeferredEndTexture.mImage,
						   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, this->mTexture1->mImage,
						   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageCopyRegion);
		}
		VulkanRenderer::GetRenderer()->EndSingleTimeCommands(commandBuffer);

		// VulkanRenderer::GetRenderer()->TransitionImageLayout(VulkanRenderer::GetRenderer()->mDeferredFinalImage,
		// VulkanRenderer::GetRenderer()->mFinalDeferredFormat, VK_IMAGE_LAYOUT_UNDEFINED,
