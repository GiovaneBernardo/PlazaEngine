...
namespace Plaza {
	struct DebugLine {
		DebugLine(const glm::vec3& start, const glm::vec3& end, float thickness, const PlColor& color)
			: mStart(start), mEnd(end), mThickness(thickness), mColor(color) {}
	...
	class PLAZA_API DebugRenderer {
	  public:
		...
		void Clear();
		void AddLine(const glm::vec3& start, const glm::vec3& end, float thickness,
					 const PlColor& color = PlColor::WHITE);
		void AddBox(const glm::vec3& position, const glm::quat& rotation = glm::identity<glm::quat>(),
					const glm::vec3& scale = glm::vec3(1, 1, 1), const PlColor& color = PlColor::WHITE,
					bool solid = true);
		void AddSphere(const glm::vec3& position, const glm::quat& rotation, const glm::vec3& scale,
					   const PlColor& color, float radius, bool solid);
		void AddCircle(const glm::vec3& position, const glm::vec3& scale, const PlColor& color, float radius,
					   bool solid);
		void AddRectangle(const glm::vec3& position, const glm::quat& rotation, const glm::vec3& scale,
						  const PlColor& color, bool solid, std::array<glm::vec3, 8>& corners);
		void Terminate();

		// Overloads to make it simple to use
		inline void AddSphere(const glm::vec3& position, float radius, const PlColor& color = PlColor::WHITE,
							  bool solid = true) {
			AddSphere(position, glm::identity<glm::quat>(), glm::vec3(1.0f, 1.0f, 1.0f), color, radius, solid);
		}

		inline void AddCircle(const glm::vec3& position, float radius, const PlColor& color = PlColor::WHITE,
							  bool solid = true) {
			AddCircle(position, glm::vec3(1.0f, 1.0f, 1.0f), color, radius, solid);
		}

		inline void AddBox(const glm::vec3& position, std::array<glm::vec3, 8>& corners,
						   const PlColor& color = PlColor::WHITE, bool solid = true,
