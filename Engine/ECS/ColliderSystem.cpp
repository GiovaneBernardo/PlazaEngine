...
namespace Plaza {
	...
	void ECS::ColliderSystem::InitCollider(Collider* collider, TransformComponent* transform, RigidBody* rigidBody) {
		...
		if (!collider->material) {
			...
			if (collider->mDynamic && rigidBody) {
				collider->material = Physics::m_physics->createMaterial(
					rigidBody->mStaticFriction, rigidBody->mDynamicFriction, rigidBody->mRestitution);
			}
		}
	...
	void ECS::ColliderSystem::InitCollider(Scene* scene, uint64_t uuid) {
		ECS::ColliderSystem::InitCollider(scene->GetComponent<Collider>(uuid),
										  scene->GetComponent<TransformComponent>(uuid),
										  scene->GetComponent<RigidBody>(uuid));
	}

	void ECS::ColliderSystem::RemoveCollider() {}
	void ECS::ColliderSystem::CreateShape(Collider* collider, TransformComponent* transform,
										  ColliderShape::ColliderShapeEnum shapeEnum, Mesh* mesh) {
		if (shapeEnum == ColliderShape::ColliderShapeEnum::BOX) {
			physx::PxBoxGeometry geometry(0.5f, 0.5f, 0.5f);
			collider->AddShape(new ColliderShape(Physics::m_physics->createShape(geometry, *Physics::defaultMaterial),
												 ColliderShape::ColliderShapeEnum::BOX, 0));
		}
		else if (shapeEnum == ColliderShape::ColliderShapeEnum::SPHERE) {
			physx::PxSphereGeometry geometry(0.5f);
			collider->AddShape(new ColliderShape(Physics::m_physics->createShape(geometry, *Physics::defaultMaterial),
												 ColliderShape::ColliderShapeEnum::SPHERE, 0));
		}
		else if (shapeEnum == ColliderShape::ColliderShapeEnum::PLANE) {
			physx::PxBoxGeometry geometry(0.5f, 0.001f, 0.5f);
			collider->AddShape(new ColliderShape(Physics::m_physics->createShape(geometry, *Physics::defaultMaterial),
												 ColliderShape::ColliderShapeEnum::PLANE, 0));
		}
	...
	void ECS::ColliderSystem::UpdateAllShapesScale(Scene* scene, uint64_t uuid) {
		if (scene->HasComponent<Collider>(uuid))
			ECS::ColliderSystem::UpdateShapeScale(scene, scene->GetComponent<Collider>(uuid),
