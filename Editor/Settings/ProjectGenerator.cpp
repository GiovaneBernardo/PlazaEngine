...
namespace Plaza::Editor {
	...
	void ProjectGenerator::GenerateSolution(const std::string& solutionName, const std::string& projectName,
		...
		if (destinationFile.is_open()) {
			...
			std::string content =
				"local PlazaRootDir = \"" + PlazaRootDir + "\"\n" + "local ProjectName = \"" + ProjectName + "\"\n";
	...
	void ProjectGenerator::GenerateProject(const std::string& projectName, const std::string& outputDirectory) {
		...
		projectFile << "<Project DefaultTargets=\"Build\" ToolsVersion=\"16.0\" "
		...
		projectFile << "    </ImportGroup>\n";
		projectFile << "    <ImportGroup Label=\"PropertySheets\" "
