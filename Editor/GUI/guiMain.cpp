...
namespace Plaza {
	namespace Editor {
		...
		void Gui::Update() {
			...
			Gui::setupDockspace(Scene::GetActiveScene(), Application::Get()->mWindow->glfwWindow,
		...
		void Gui::Init(GLFWwindow* window) {
			...
			Application::Get()->mEditor->mGui.Gui::mAssetsImporter =
				new AssetsImporterWindow(GuiLayer::ASSETS_IMPORTER, false);
			Application::Get()->mEditor->mGui.Gui::mConsole = new Console(GuiLayer::CONSOLE, false);
			Application::Get()->mEditor->mGui.Gui::mRenderGraphEditor =
				new RenderGraphEditor(GuiLayer::RENDER_GRAPH_EDITOR, false);
...
#pragma endregion
		...
		void Gui::setupDockspace(Scene* scene, GLFWwindow* window, Camera* camera) {
			...
			//| ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoTitleBar; 		windowFlags |=
			//ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoBringToFrontOnFocus |
			//ImGuiConfigFlags_DockingEnable | ImGuiWindowFlags_NoScrollbar;
		...
		inline void Gui::beginScene(Scene* scene, Camera& camera) {
			...
			if (isSceneOpen) {
				...
				ImGui::Image(ImTextureID(Application::Get()->mRenderer->GetFrameImage()),
							 ImGui::imVec2(appSizes.sceneSize), uv0, uv1);
				// ImGui::Image(ImTextureID(Application::Get()->textureColorbuffer), ImGui::imVec2(appSizes.sceneSize),
				// uv0, uv1);
				...
				if (ImGui::BeginDragDropTarget()) {
					if (const ImGuiPayload* payload = ImGui::AcceptDragDropPayload(scenePayloadName.c_str())) {
						if (payload->DataSize == sizeof(Editor::File)) {
							...
							//
							//	ModelLoader::LoadImportedModelToScene(ModelSerializer::ReadUUID(file->directory),
							//file->directory);
							// }
							delete (file);
						}
		...
		inline void Gui::beginEditor(Scene* scene, Camera& camera) {
			...
			ImGui::Image(mShowSelectedImageInEditorView && Gui::mSelectedImageInspector
							 ? Gui::mSelectedImageInspector->mTextureID
							 : ImTextureID(Application::Get()->mRenderer->GetFrameImage()),
						 ImGui::imVec2(appSizes.sceneSize), uv0, uv1);
			...
			if (selectedGameObject && scene->GetComponent<TransformComponent>(selectedGameObject->uuid) != nullptr &&
				selectedGameObject->parentUuid != 0) {
		...
		void Gui::beginHierarchyView(Scene* scene) {
			...
			// ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(1.0f, 0.0f, 0.0f, 1.0f)); // Set window background to
			// red//
		...
		void Gui::beginInspector(Scene* scene, Camera camera) {
			...
			ImGuiWindowFlags sceneWindowFlags = ImGuiWindowFlags_NoFocusOnAppearing | ImGuiConfigFlags_DockingEnable;
			ImGuiWindowFlags inspectorWindowFlags = ImGuiWindowFlags_NoNavFocus |
		...
		void Gui::beginImageInspector(Scene* scene, Camera camera) {
			...
			Gui::imageSize = ImVec2(
				200 * (Application::Get()->appSizes->sceneSize.x / Application::Get()->appSizes->sceneSize.y), 200);
			...
			if (!Gui::mImageInspectorShowAllImages && Gui::mSelectedImageInspector) {
				ImGui::Image(Application::Get()->mRenderer->GetTrackedImageID(Gui::mSelectedImageInspector),
							 Gui::imageSize, uv0, uv1);
				...
				if (ImGui::DragInt("Layer", &tempLayerLevel, 1.0f, 0,
								   Gui::mSelectedImageInspector->mTextureInfo.mLayersCount)) {
			...
			for (unsigned int i = 0; i < Application::Get()->mRenderer->mTrackedImages.size(); ++i) {
				if (Gui::mImageInspectorShowAllImages) {
					ImGui::Image(Application::Get()->mRenderer->GetTrackedImageID(
									 Application::Get()->mRenderer->GetTrackedImage(i)),
								 Gui::imageSize, uv0, uv1);
				}
				else {
			...
			if (ImGui::Begin("Assets Viewer", &Gui::isSceneOpen, windowFlags)) {
				...
				while (clipper.Step()) {
					for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
						...
						if (ImGui::TreeNodeEx(value->mAssetPath.filename().string().c_str(),
			...
			if (ImGui::Begin("Profiler", &Gui::isSceneOpen, windowFlags)) {
				for (const auto& [key, value] : Profiler::sProfilers) {
					if (ImGui::TreeNode(key.c_str())) {
						...
						ImGui::SameLine();
						ImGui::Text(std::string(std::to_string(std::chrono::duration_cast<std::chrono::milliseconds>(
						...
						for (const SectionProfiler& section : value->mSections) {
							ImGui::Text(std::string(section.mName + " " +
													std::to_string(section.mDurationNanoSeconds / 1000 / 100) +
													" Milliseconds")
											.c_str());
						}
						ImGui::TreePop();
