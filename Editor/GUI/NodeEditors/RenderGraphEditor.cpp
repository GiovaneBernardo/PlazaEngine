...
namespace Plaza::Editor {
	void RenderGraphEditor::Init() {
		...
		// this->AddInputPin(textureNode, Pin(0, "Name", PinType::String,
		// PinKind::Constant)).SetValue<std::string>("Texture"); this->AddOutputPin(textureNode, Pin(0, "Out",
		// PinType::Object, PinKind::Output)).value.SetType<Texture>(); textureNode.processFunction = [](Node& node) {
		//
		//	};
		...
		this->AddInputPin(textureBindingNode, Pin(0, "Descriptor Count", PinType::Int, PinKind::Constant))
			.SetValue<int>(0);
		...
		this->AddInputPin(textureBindingNode, Pin(0, "Buffer Type", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlBufferType>(0);
		this->AddInputPin(textureBindingNode, Pin(0, "Render Stage", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlRenderStage>(0);
		this->AddInputPin(textureBindingNode, Pin(0, "Initial Layout", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlImageLayout>(0);
		this->AddInputPin(textureBindingNode, Pin(0, "Base Mip Level", PinType::Int, PinKind::Constant))
			.SetValue<int>(0);
		this->AddInputPin(textureBindingNode, Pin(0, "Base Layer Level", PinType::Int, PinKind::Constant))
			.SetValue<int>(0);
		this->AddInputPin(textureBindingNode, Pin(0, "Resource Name", PinType::String, PinKind::Constant))
			.SetValue<std::string>("");
		this->AddOutputPin(textureBindingNode, Pin(0, "Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaTextureBinding>();
		...
		this->AddInputPin(bufferBindingNode, Pin(0, "Descriptor Count", PinType::Int, PinKind::Constant))
			.SetValue<int>(0);
		this->AddInputPin(bufferBindingNode, Pin(0, "Binding", PinType::Int, PinKind::Constant)).SetValue<int>(0);
		this->AddInputPin(bufferBindingNode, Pin(0, "Render Stage", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlRenderStage>(0);
		this->AddInputPin(bufferBindingNode, Pin(0, "Resource Name", PinType::String, PinKind::Constant))
			.SetValue<std::string>("");
		this->AddOutputPin(bufferBindingNode, Pin(0, "Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaBufferBinding>();
		...
		this->AddInputPin(renderPassNode, Pin(0, "Pass Name", PinType::String, PinKind::Constant))
			.SetValue<std::string>("Render Pass");
		this->AddInputPin(renderPassNode, Pin(0, "Stages", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlRenderStage>(0);
		this->AddInputPin(renderPassNode, Pin(0, "Render Mode", PinType::Enum, PinKind::Constant))
			.SetEnumValue<PlRenderPassMode>(0);
		this->AddInputPin(renderPassNode, Pin(0, "Multi view count", PinType::Int, PinKind::Constant)).SetValue<int>(0);
		this->AddInputPin(renderPassNode, Pin(0, "Render Size", PinType::Vector2, PinKind::Constant))
			.SetValue<glm::vec2>(glm::vec2(1920.0f, 1080.0f));
		this->AddInputPin(renderPassNode, Pin(0, "Dispatch Size", PinType::Vector3, PinKind::Constant))
			.SetValue<glm::vec3>(glm::vec3(0.0f));
		this->AddInputPin(renderPassNode, Pin(0, "Flip Viewport", PinType::BoolType, PinKind::Constant)).SetValue(true);
		this->AddInputPin(renderPassNode, Pin(0, "Input Textures", PinType::Array, PinKind::Input))
			.SetValue<PlazaTextureBinding>(PlazaTextureBinding());
		renderPassNode.inputs.back().isVector = true;
		this->AddInputPin(renderPassNode, Pin(0, "Input Buffers", PinType::Array, PinKind::Input))
			.SetValue<PlazaBufferBinding>(PlazaBufferBinding());
		renderPassNode.inputs.back().isVector = true;
		this->AddInputPin(renderPassNode, Pin(0, "Output Textures", PinType::Array, PinKind::Input))
			.SetValue<PlazaTextureBinding>(PlazaTextureBinding());
		renderPassNode.inputs.back().isVector = true;
		this->AddInputPin(renderPassNode, Pin(0, "Output Buffers", PinType::Array, PinKind::Input))
			.SetValue<PlazaBufferBinding>(PlazaBufferBinding());
		renderPassNode.inputs.back().isVector = true;
		this->AddInputPin(renderPassNode, Pin(0, "Pipelines", PinType::Array, PinKind::Input))
			.SetValue<PlPipelineCreateInfo>(PlPipelineCreateInfo());
		renderPassNode.inputs.back().isVector = true;
		this->AddOutputPin(renderPassNode, Pin(0, "Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaRenderPass>();
		this->AddOutputPin(renderPassNode, Pin(0, "Input Textures Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaTextureBinding>();
		this->AddOutputPin(renderPassNode, Pin(0, "Input Buffers Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaBufferBinding>();
		this->AddOutputPin(renderPassNode, Pin(0, "Output Textures Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaTextureBinding>();
		this->AddOutputPin(renderPassNode, Pin(0, "Output Buffers Out", PinType::Object, PinKind::Output))
			.value.SetType<PlazaBufferBinding>();
		...
		this->InitNodeEditor();
		// this->SpawnNode(typeid(PlPipelineCreateInfo).name()).inputs[0].SetValue<std::string>("Draw to SwapChain
		// Pass");
	};
	...
	void RenderGraphEditor::Process() {
		...
		for (auto& [key, node] : mNodesData.mNodes) {
			...
			if (value.type() == typeid(PlPipelineCreateInfo)) {
				...
				std::vector<PlPipelineCreateInfo> pipelinesInfo =
					node.outputs[5].GetValue<std::vector<PlPipelineCreateInfo>>();
		...
		Application::Get()->mRenderer->UpdateImGuiDisplayTexture(
			Application::Get()->mRenderer->mRenderGraph->GetTexture<VulkanTexture>("FinalTexture"));

		// AssetsSerializer::SerializeFile<VulkanRenderGraph>(*renderGraph, FileDialog::SaveFileDialog(""),
		// Application::Get()->mSettings.mRenderGraphSerializationMode);

		PL_CORE_INFO("Alright");
	}

	void RenderGraphEditor::LoadRenderGraphNodes(PlazaRenderGraph* renderGraph) { PL_CORE_INFO("Loaded"); }
